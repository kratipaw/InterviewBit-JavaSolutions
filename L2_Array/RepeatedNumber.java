...
public class RepeatedNumber {
	...
	public static void main(String[] args) {
		...
		System.out.println("Repeated num  : " + repeatedNum(list));
		...
		System.out.println("Repeated Num : " + repeatedNum(arr));
	}
	
	private static int repeatedNum(final List<Integer> a) {
		List<Integer> temp = new ArrayList<Integer>(a);
			
			
		for (int i = 0; i < temp.size(); i++) {
			if(temp.get(Math.abs(temp.get(i)))>=0)
				temp.set(Math.abs(temp.get(i)), -temp.get(Math.abs(temp.get(i))));
			else
				return Math.abs(temp.get(i));
		}
		
		return -1;
	}
	
	private static int repeatedNum(int arr[]) {
